...
@RequestMapping(path = "/api/v1/menu") // This means URL's start with /demo (after Application path)
public class ApiV1MenuController {

	@Autowired
	private MenuRepository userRepository2;
	 ...
	 */
	@PostMapping()
	public Menu create(@RequestBody Menu users) {
	 ...
	 */
	@GetMapping()
	public Iterable<Menu> retrieveAll() {
	 ...
	 */
	@GetMapping("ids")
	public Iterable<Menu> retrieveIds(@RequestBody Map<String, List<Long>> map  ) {
	 ...
	 */
	@GetMapping("{id}")
	public Optional<Menu> retrieveId(@PathVariable(name = "id") Long id) {
	 ...
	 */
	@PutMapping("{id}")
	public Menu update(@PathVariable(name = "id") Long id, @RequestBody Menu users) {
		return userRepository2.save(users);
	}
	...
	@PatchMapping("{id}")
	@Deprecated
	public Menu updateName(@PathVariable(name = "id") Long id, @RequestBody Menu users) {
		return userRepository2.save(users);
	}
